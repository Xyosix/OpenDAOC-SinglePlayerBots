...
namespace DOL.GS.Scripts
{
    ...
    public class StatCalculator : PropertyCalculator
    {
        ...
        public override int CalcValue(GameLiving living, eProperty property)
        {
            ...
            else if (living is MimicNPC mimic && living.Level > 1)
            {
                if (property == (eProperty)mimic.CharacterClass.ManaStat)
                {
                    if (IsClassAffectedByAcuityAbility(mimic.CharacterClass))
                        abilityBonus += mimic.AbilityBonus[(int)eProperty.Acuity];
                }

                deathConDebuff = mimic.TotalConstitutionLostAtDeath;
                livingToCheck = mimic;
            }
        ...
        public override int CalcValueFromBuffs(GameLiving living, eProperty property)
        {
            ...
            else if (living is MimicNPC mimic && living.Level > 1   )
            {
                if (property == (eProperty)mimic.CharacterClass.ManaStat)
                {
                    if (mimic.CharacterClass.ClassType == eClassType.ListCaster)
                        specBuffBonus += mimic.BaseBuffBonusCategory[(int)eProperty.Acuity];
                }
            }
